...
<script>
	...
	$(function () {
		...
		$(document).on("click", ".person_container .person .submitperson", function(){
			...
			else if (tp_person == "kids")
			{
				if (jsOld >= 15 || jsOld < 5)
				{
					$(datarozhdenia).css("border", "1px solid red");
					error = true;
					alert("Ребенок должен быть старше 5 лет и младше 15!");
				}
			}
			...
			if(tp_person == "adult")
			{
				...
				if(isBesplatno == 0) {
				    console.log("2st");
					if(kids_person_i/adult_person_i >= 6) 
					{
					isBesplatno = 1;
					besplatno = "besplatno";
					console.log(besplatno);
					} 
				}
		...
		$(document).on("click", ".person_container .cl", function(){
			...
			if (kids_person_i < 6)
			{	
				alert("Должно быть не менее 6-ти детей в школьном посещении");
				kids_person_i = 6;
			}
			
			else if (adult_person_i == 0) {
				alert("В школьном посещении обязательно должен быть хотя бы один взрослый");
				adult_person_i = 1;
			}
			else if(price == 0) {
				alert("На группу из 6-ти детей должен быть хотя бы один сопровождающий. Попробуйте удалить другого взрослого.");
			}
